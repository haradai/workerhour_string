...
d=Date.today
P.each_char{|c|h[c]=0}
w=(Time.now.to_i-Time.new(d.year,d.month,d.day,9).to_i)/60
...
r=''
h.sort{|(k,v),(l,w)|P.index(k)<=>P.index(l)}.each{|k,v|m=(w*v.to_f/l).to_i;t=(m+q(m%U))/60.0;r<<k+"#{(t==t.to_i)?t.to_i: t}"}
puts r
